 ...
 */
class BillingDeliveryOrderController extends AppController
{
    ...
    public function add(){
        ...
        $do_no = $this->BdoReport->find()->count();
    ...
    public function addItems(){
        ...
        $nbdoTable = TableRegistry::get('BdoReport');
    ...
    public function approve($id = null){
        $this->loadModel('BdoReport');
        $nbdo = $this->BdoReport->get($id, [
            'contain' => []
        ]);
        if ($this->request->is(['patch', 'post', 'put'])) {
            $nbdo = $this->BdoReport->patchEntity($nbdo, $this->request->data);
            if ($this->BdoReport->save($nbdo)) {
